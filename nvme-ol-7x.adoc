...
summary: 为 Oracle Linux 7x 和ONTAP设置 VMe/FC 主机配置，并提供示例 
---
...
了解如何为 Oracle Linux 7x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-ol-supported-features.html["Oracle Linux ONTAP支持和功能"]。
...
.步骤
. 在服务器上安装Oracle Linux 7x。安装完成后，请确认您运行的是指定的 Oracle Linux 7x 内核。
+
[source, cli]
----
uname -r
...
----
. 在 Oracle Linux 7x 主机上，检查 `hostnqn`字符串 `/etc/nvme/hostnqn`：
+
[source, cli]
----
