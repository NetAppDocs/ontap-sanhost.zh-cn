...
---
= 使用 NVMe-oF 为 ONTAP 存储配置 Rocky Linux 9.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 9.0 主机支持 NVMe/FC 和 NVMe/TCP 协议，并支持非对称命名空间访问 (ANA)。ANA相当于 iSCSI 和 FCP 环境中的非对称逻辑单元访问 (ALUA) 多路径，是通过内核 NVMe 多路径功能实现的。

有关支持的配置的更多详细信息，请参见link:https://mysupport.netapp.com/matrix/["互操作性表工具"^]。

.关于此任务
您可以将以下支持和功能与 Rocky Linux 9.0 的 NVMe-oF 主机配置一起使用。在开始配置过程之前、您还应查看已知限制。
...
* 可用功能：
+
** Rocky Linux 9.0 默认为 NVMe 命名空间启用内核 NVMe 多路径，无需明确设置。
...
使用以下步骤验证最低支持的 Rocky Linux 9.0 软件版本。

.步骤
. 在服务器上安装 Rocky Linux 9.0。安装完成后，验证您运行的是指定的 Rocky Linux 9.0 内核：
+
[source, cli]
----
uname -r
...
带有 ONTAP 的 Rocky Linux 9.0 的 NVMe-oF 主机配置存在以下已知问题：
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 9.0 NVMe-oF 主机创建重复的持久发现控制器 | 在基于网络结构的NVMe (NVMe-oF)主机上、您可以使用"nvme discover -p"命令创建永久性发现控制器(POC)。使用此命令时、每个启动程序-目标组合只应创建一个PDC。但是，如果您在 NVMe-oF 主机上运行 ONTAP 9.10.1 和 Rocky Linux 9.0，则每次执行“nvme discover -p”时都会创建重复的 PDC。这会导致不必要地使用主机和目标上的资源。 
|===