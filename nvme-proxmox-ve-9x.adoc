...
summary: 如何为 Proxmox VE 9.x 配置 NVMe-oF 主机和ONTAP存储 
---
...
了解如何为 Proxmox VE 9.x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-proxmox-supported-features.html["ONTAP支持和功能"^]。
...
.步骤
. 在服务器上安装 Proxmox VE 9.x。安装完成后，请确认您运行的是所需的 Proxmox VE 9.x 内核：
+
[source, cli]
----
uname -r
...
----
. 修改规则，使  `queue-depth` 更改为  `round-robin`，如以下示例中所示：
+
[listing]
----
...
----
. 验证子系统的当前 iopolicy。将 <subsystem> 替换为子系统的名称，例如 nvme-subsys0。
+
[source, cli]
----
