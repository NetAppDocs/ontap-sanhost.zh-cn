...
summary: 介绍如何在 ONTAP 中使用 Red Hat Enterprise Linux 8.2 
---
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将Red Hat Enterprise Linux 8.2配置为目标ONTAP。
...
对于Red Hat Enterprise Linux (RHEL) 8.2、请参见 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。RHEL 8.2包含识别和正确管理ONTAP LUN所需的所有设置。
...
RHEL 8.2 操作系统经过编译，可识别 ONTAP LUN 并自动正确设置所有配置参数。
