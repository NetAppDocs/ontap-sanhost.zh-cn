...
---
= 使用ONTAP存储对 NVMe-oF 进行故障排除（适用于 Linux 主机实用程序）
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
对配备ONTAP存储的 Oracle Linux、RHEL、Rocky Linux 和 SUSE Linux Enterprise Server 主机进行 NVMe-oF 故障排查。

在开始故障排除之前，请确认您运行的配置符合以下要求： https://mysupport.netapp.com/matrix/#welcome["互操作性表工具"^] 然后按照说明进行下一步操作，以调试任何主机端问题。


NOTE: 故障排除说明不适用于 AIX、ESXi、Proxmox 和 Windows 主机。
...
.步骤
为 LPFC 或 Qlogic (Qla2xxx) 设置详细日志记录。
...
如果您仍然遇到问题，请收集以下文件和命令输出，并联系 link:https://mysupport.netapp.com["NetApp 支持"^] 以进行进一步的分类：
