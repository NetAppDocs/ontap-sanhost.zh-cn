...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | 要支持Emulex HBA适配器发现、NetApp Linux Host Utilities CLI需要其他库软件包依赖项 | 在RHEL 9.1中、NetApp Linux SAN主机实用程序CLI `sanlun fcp show adapter -v` 失败、因为找不到支持HBA发现的库软件包依赖关系。 
|===