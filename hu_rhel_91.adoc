...
summary: 介绍如何在ONTAP 中使用Red Hat Enterprise Linux 9.1 
---
= 将Red Hat Enterprise Linux 9.1与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将ONTAP配置为目标Red Hat Enterprise Linux 9.1。



== 安装 Linux Unified Host Utilities

上提供了 NetApp Linux Unified Host Utilities 软件包 link:https://mysupport.netapp.com/site/downloads["NetApp 支持站点"^] 在 64 位 .rpm 文件中。

NetApp强烈建议安装Linux统一主机实用程序、但并非强制要求。这些实用程序不会更改 Linux 主机上的任何设置。这些实用程序可改进管理并帮助 NetApp 客户支持收集有关您的配置的信息。

.您需要的内容
如果您当前安装了某个版本的Linux Unified Host Utilities、则应升级或删除该版本、并按照以下步骤安装最新版本。

.步骤
. 从下载 64 位 Linux Unified Host Utilities 软件包 link:https://mysupport.netapp.com/site/downloads["NetApp 支持站点"] 主机。
. 安装软件包：
...
对于Red Hat Enterprise Linux (RHEL) 9.1 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。RHEL 9.1使用识别和正确管理ONTAP LUN所需的所有设置进行编译。
...
RHEL 9.1操作系统经过编译、可识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | 要支持Emulex HBA适配器发现、NetApp Linux Host Utilities CLI需要其他库软件包依赖项 | 在RHEL 9.1中、NetApp Linux SAN主机实用程序CLI `sanlun fcp show adapter -v` 失败、因为找不到支持HBA发现的库软件包依赖关系。 | 不适用 
|===