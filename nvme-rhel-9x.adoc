...
summary: 如何为 RHEL 9.x 配置 NVMe-oF 主机（使用ONTAP） 
---
= 配置 RHEL 9.x 以使用 NVMe-oF 和ONTAP存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 RHEL 9.x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rhel-supported-features.html["RHEL ONTAP支持和功能"]。

NVMe-oF 与 RHEL 9.x 存在以下已知限制：
...
.步骤
. 在服务器上安装 RHEL 9.x。安装完成后，请确认您运行的是所需的 RHEL 9.x 内核：
+
[source, cli]
----
uname -r
...
.RHEL 9.x 版本的 iopolicy 更改
* 从 RHEL 9.7 开始，默认 iopolicy 为 queue-depth。
* 在 RHEL 9.6 中，round-robin 是默认的 iopolicy，但 queue-depth 可作为可配置选项使用。
* 在 RHEL 9.6 之前，循环是唯一受支持的 NVMe-oF iopolicy。


如果使用 RHEL 9.6 并希望将 iopolicy 更改为 `queue-depth`，请按如下所示修改 udev 规则文件：
...
.NVMe/FC
--
以下示例输出显示了在双节点 ONTAP 控制器上托管的命名空间，适用于 AFF、FAS 和 ASA 系统以及具有 NVMe/FC 的 ASA r2 系统。
...
.NVMe/TCP
--
以下示例输出显示了托管在 AFF、FAS 和 ASA 系统以及 ASA r2 系统的双节点 ONTAP 控制器上使用 NVMe/TCP 的命名空间。
...
支持通过 NVMe/TCP 在 RHEL 9.x 主机和ONTAP控制器之间进行安全的带内身份验证。
