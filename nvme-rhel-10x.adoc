...
.RHEL 10.x 版本的 iopolicy 更改
* 从 RHEL 10.1 开始，默认 iopolicy 为 queue-depth。
* 在 RHEL 10.0 中，round-robin 是默认的 iopolicy，但 queue-depth 是可配置的选项。


如果使用 RHEL 10.0 并希望将 iopolicy 更改为 queue-depth，请修改 udev rules 文件，如下所示：
...
.步骤
. 验证主机上是否正确显示了相应ONTAP命名空间的 NVMe-oF 设置（例如，将型号设置为NetApp ONTAP Controller，并将负载均衡 iopolicy 设置为 queue-depth）：
+
.. 显示子系统：
+
[source, cli]
