...
summary: 如何为 RHEL 10x 配置 NVMe-oF 主机（使用ONTAP） 
---
...
了解如何为 RHEL 10x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rhel-supported-features.html["RHEL ONTAP支持和功能"]。
...
.步骤
. 在服务器上安装 RHEL 10x。安装完成后，请确认您运行的是所需的 RHEL 10x 内核：
+
[source, cli]
----
uname -r
