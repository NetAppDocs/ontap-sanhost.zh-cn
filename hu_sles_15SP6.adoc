...
summary: 介绍如何将SUSE Linux Enterprise Server 15 SP6与ONTAP结合使用 
---
= 将SUSE Linux Enterprise Server 15 SP6与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置来配置以ONTAP为目标的SUSE Linux Enterprise Server 15 SP6。
...
.您需要的内容
您的配置支持SAN启动。请参见link:https://mysupport.netapp.com/matrix/imt.jsp?components=84067;&solution=1&isHWU&src=IMT["NetApp 互操作性表工具"^]以验证您的操作系统、HBA、HBA固件以及HBA启动BIOS和ONTAP版本是否受支持。
...
对于SUSE Linux Enterprise Server 15 SP6、该 `/etc/multipath.conf`文件必须存在。您无需更改此文件、因为SUSE Linux Enterprise Server 15 SP6已编译为识别和正确管理ONTAP LUN所需的所有设置。
...
SUSE Linux Enterprise Server 15 SP6操作系统可识别ONTAP LUN、并自动为ASA和非ASA配置正确设置所有配置参数。您可以使用以下建议设置进一步优化主机配置的性能。
...
----
.示例
在以下示例中、您将确定设备的WWID并将其添加到文件中 `multipath.conf`。
