...
summary: 介绍如何将Oracle Linux 9.2与ONTAP结合使用 
---
= 将Oracle Linux 9.2与ONTAP结合使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将使用ONTAP的Oracle Linux 9.2配置为目标。
...
对于OL 9.2、需要 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。OL 9.2包含识别和正确管理ONTAP LUN所需的所有设置。
...
经过编译的Oracle Linux 9.2操作系统可识别ONTAP LUN、并自动为ASA和非ASA配置正确设置所有配置参数。您可以使用以下建议设置进一步优化主机配置的性能。
...
NOTE: 要配置Oracle Linux 9.2 Red Hat Enterprise Kernel (RHCK)、请使用link:hu_rhel_92.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 9.2的。
...
Oracle Linux 9.2 with ONTAP版本存在以下已知问题：
