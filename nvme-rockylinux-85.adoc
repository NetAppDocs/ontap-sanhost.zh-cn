...
---
= 使用ONTAP存储为 NVMe-oF 配置 Rocky Linux 8.5
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 Rocky Linux 8.5 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支持和功能"]。

*带有 Rocky Linux 8.5 的 NVMe-oF 具有以下已知限制：*

* 目前不支持使用 NVMe-oF 协议的 SAN 启动。
* 在 Rocky Linux 8.5 中的 NVMe-oF 主机上，内核 NVMe 多路径默认处于禁用状态；您必须手动启用它。
...
.步骤
. 在服务器上安装 Rocky Linux 8.5。安装完成后，验证您正在运行所需的 Rocky Linux 8.5 内核：
+
[source, cli]
----
uname -r
