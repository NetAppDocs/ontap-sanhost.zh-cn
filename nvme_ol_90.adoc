...
--
.步骤
. OL 9.0 GA内核中包含的本机内置qla2xxx驱动程序包含对ONTAP支持至关重要的最新上游修复程序。验证您是否正在运行受支持的适配器驱动程序和固件版本：
+
[listing]
----
...
使用ONTAP的Oracle Linux 9.0的NVMe-oF主机配置存在以下已知问题：
...
| 1517321 | Oracle Linux 9.0 NVMe-oF主机会创建重复的永久性发现控制器 | 在Oracle Linux 9.0基于网络结构的NVMe (NVMe-oF)主机上、您可以使用 `nvme discover -p` 用于创建永久性发现控制器(POC)的命令。使用此命令时、每个启动程序-目标组合只应创建一个PDC。但是、如果在运行ONTAP 9.10.1和Oracle Linux 9.0的NVMe-oF主机上运行、则每次都会创建一个重复的PDC `nvme discover -p` 已执行。这会导致不必要地使用主机和目标上的资源。 
|===