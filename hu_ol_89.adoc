...
summary: 介绍如何将Oracle Linux 8.9与ONTAP结合使用 
---
= 将Oracle Linux 8.9与ONTAP结合使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置来配置以ONTAP为目标的Oracle Linux 8.9。
...
.您需要的内容
您的配置支持SAN启动。请参见link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["NetApp 互操作性表工具"^]以验证您的操作系统、HBA、HBA固件以及HBA启动BIOS和ONTAP版本是否受支持。
...
对于Oracle Linux (OL) 8.9、该 `/etc/multipath.conf`文件必须存在。您无需更改此文件、因为Oracle Linux 8.9会使用识别和正确管理ONTAP LUN所需的所有设置进行编译。
...
NOTE: 一个LUN不应要求四个以上的路径。在存储故障期间，超过八个路径可能会出现发生原因路径问题。
...
经过编译的Oracle Linux 8.9操作系统可识别ONTAP LUN、并自动为ASA和非ASA配置设置正确的配置参数。您可以使用以下建议设置进一步优化主机配置的性能。
...
NOTE: 要配置Oracle Linux 8.9 RedHat Enterprise Kernel、请使用link:hu_rhel_89.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 8.9的。



== KVM设置

您还可以使用建议的设置来配置基于内核的虚拟机(KVM)。配置KVM不需要进行任何更改、因为此LUN已映射到虚拟机管理程序。



== 已知问题

具有ONTAP版本的Oracle Linux 8.9没有已知问题。
