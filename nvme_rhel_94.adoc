...
NVMe/TCP没有自动连接功能。因此、我们需要手动执行NVMe/TCP连接或全部连接功能来发现NVMe/TCP子系统和命名共享。
...
NOTE: 从RHEL 9.4开始、这是NVMe/TCP的默认设置 `ctrl_loss_tmo` 超时已关闭、这意味着对重试次数没有限制(无限期重试)。因此、您无需手动配置特定 `ctrl_loss_tmo` 使用NVMe connect或NVMe Connect-all命令时的超时持续时间(选项-l)。使用此默认行为时、NVMe/TCP控制器在发生路径故障时不会发生超时、并会无限期保持连接。
