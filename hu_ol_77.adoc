...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将Oracle Linux 7.7与ONTAP配置为目标。
...
经过编译的Oracle Linux 7.7操作系统可识别ONTAP LUN并自动正确设置所有配置参数。 `multipath.conf`要启动多路径守护进程、必须存在该文件。如果此文件不存在、您可以使用以下命令创建一个空的零字节文件：
...
NOTE: 要配置Oracle Linux 7.7 Red Hat Enterprise Kernel (RHCK)、请使用link:hu_rhel_77.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 7.7的。



== 已知问题

具有ONTAP版本的Oracle Linux 7.7存在以下已知问题：
