...
summary: 介绍如何将 HP-UX Host Utilities 6.0 与ONTAP配合使用 
---
= 使用 HP-UX Host Utilities 命令验证ONTAP存储配置
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
使用 HP-UX Host Utilities 6.0 示例命令参考，通过 Host Utilities 工具对ONTAP存储配置进行端到端验证。
...
检索连接到主机的所有 SVM 目标 LIF WWPN 的列表。
