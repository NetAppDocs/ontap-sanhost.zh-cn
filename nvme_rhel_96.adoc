...
summary: 如何使用ONTAP为RHEL 9.6配置NVMe-oF主机 
---
= 适用于采用ONTAP的RHEL 9.6的NVMe-oF主机配置
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.关于此任务
您可以在适用于Red Hat Enterprise Linux (RHEL) 9.6的NVMe-oF主机配置中使用以下支持和功能。在开始配置过程之前、您还应查看已知限制。
...
** 默认情况下、RHEL 9.6会为NVMe命名空间启用内核NVMe多路径、从而无需显式设置。
** 从 ONTAP 9.12.1 开始，引入了对 NVMe/TCP 的安全带内身份验证的支持。您可以在 RHEL 9.6 中使用 NVMe/TCP 的安全带内身份验证。
+
...
使用以下步骤验证最低支持的 RHEL 9.6 软件版本。

.步骤
. 在服务器上安装RHEL 9.6。安装完成后、验证是否正在运行指定的RHEL 9.6内核：
+
[source, cli]
----
uname -r
...
----
. 在RHEL 9.6主机上，检查以下位置的hostnqn字符串 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
使用 RHEL 9.6，  `nvmefc-boot-connections.service`和 `nvmf-autoconnect.service` NVMe/FC 中包含的启动服务 `nvme-cli`软件包会在系统启动时自动启用。系统启动完成后，请验证启动服务是否已启用。
...
从 ONTAP 9.12.1 开始，RHEL 9.6 主机和 ONTAP 控制器之间通过 NVMe/TCP 支持安全带内身份验证。
