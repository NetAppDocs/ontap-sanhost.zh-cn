...
NOTE: AIX 要求在启用 MPIO 之前，SAN 启动磁盘只能看到一条路径。安装或迁移 AIX 启动磁盘通常需要临时限制主机的 FC 分区，以包含单个 FC 启动器和单个 FC 目标。安装主机实用程序（包括 PCM 包）后，您可以重新启动操作系统以完全启用 MPIO。此时，将额外的 FC 启动器和 FC 目标添加到区域。然后，运行 `cfgmgr -v`和 `bosboot -a`以更新启动配置以包括所有可用路径。
