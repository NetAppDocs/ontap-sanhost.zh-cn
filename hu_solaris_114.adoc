...
+
** 更改x86和SPARC系统的光纤通道和SCSI驱动程序的设置
...
默认情况下，如果 LUN 的所有路径都丢失，则 Solaris 操作系统将在 20 秒后使 I/O 失败。这由控制 `fcp_offline_delay` 参数。的默认值 `fcp_offline_delay` 适用于标准ONTAP 集群。但是、在MetroCluster 配置中、的值 `fcp_offline_delay` 必须增加到* 120秒*、以确保I/O在包括计划外故障转移在内的操作期间不会过早超时。有关默认设置的其他信息和建议更改、请参见NetApp知识库文章 https://["MetroCluster 配置中的 Solaris 主机支持注意事项"^]。



== Oracle Solaris 虚拟化

* Solaris 虚拟化选项包括 Solaris 逻辑域（也称为 LDOM 或适用于 SPARC 的 Oracle VM Server ）， Solaris 动态域， Solaris 区域和 Solaris 容器。这些技术通常已更名为"Oracle虚拟机"、尽管它们基于不同的架构。
