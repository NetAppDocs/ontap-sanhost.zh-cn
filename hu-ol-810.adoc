...
.开始之前
使用link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["NetApp 互操作性表工具"^]验证您的操作系统、HBA、HBA固件、HBA启动BIOS和ONTAP版本是否受支持。
...
NOTE: 要配置Oracle Linux 8.10 RedHat Enterprise Kernel、请使用link:hu_rhel_810.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 8.10的。



== KVM设置

您还可以使用<<recommended_settings_ol_810,建议设置>>配置基于内核的虚拟机(KVM)。配置KVM不需要进行任何更改、因为此LUN已映射到虚拟机管理程序。
