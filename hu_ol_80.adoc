...
.您需要的内容
如果您决定使用 SAN 启动，则配置必须支持它。您可以使用 link:https://mysupport.netapp.com/matrix/imt.jsp?components=89523;&solution=1&isHWU&src=IMT["NetApp 互操作性表工具"^] 验证您的操作系统， HBA ， HBA 固件和 HBA 启动 BIOS 以及 ONTAP 版本是否受支持。
...
NOTE: 要配置Oracle Linux 8.0 Red Hat Enterprise Kernel (RHCK)、请使用link:hu_rhel_80.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 8.0的。



== 已知问题

Oracle Linux 8.0 with ONTAP发行版没有已知问题。


NOTE: 有关 Oracle Linux （ Red Hat 兼容内核）的已知问题，请参见 link:hu_rhel_80.html#known-problems-and-limitations["已知问题"] 适用于 Red Hat Enterprise Linux （ RHEL ） 8.0 。
