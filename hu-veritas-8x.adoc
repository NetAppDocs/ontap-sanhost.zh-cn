...
summary: 使用 Veritas Infoscale 8 for RHEL 和 Oracle Linux 主机的 Linux 主机实用程序来帮助您管理ONTAP LUN 的 FC、FCoE 和 iSCSI 协议操作。 
---
= 配置 Veritas Infoscale 8 以支持 FC、FCoE 和 iSCSI 以及ONTAP存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 软件为连接到ONTAP存储的 Linux 主机提供管理和诊断工具。使用 Veritas Infoscale 8 for Oracle Linux（基于 Red Hat 兼容内核）、Red Hat Enterprise Linux (RHEL) 和 SUSE Linux Enterprise Server 主机上的 Linux 主机实用程序，以支持使用ONTAP LUN 管理 FC、FCoE 和 iSCSI 协议操作。
...
使用 Veritas 动态多路径 (VxDMP) 和 Veritas Infoscale 8 来管理ONTAP LUN。
