...
summary: 介绍如何在ONTAP 中使用Oracle Linux 8.6 
---
= 将Oracle Linux 8.6与ONTAP结合使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将使用ONTAP的Oracle Linux 8.6配置为目标。
...
对于Oracle Linux (OL) 8.6、必须存在`/etc/multipath.conf`文件、但您不需要对该文件进行特定更改。OL 8.6使用识别和正确管理ONTAP LUN所需的所有设置进行编译。
...
Oracle Linux 8.6操作系统经过编译、可以识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
...
NOTE: 要配置Oracle Linux 8.6 Red Hat Enterprise Kernel (RHCK)、请使用link:hu_rhel_86.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 8.6的。
...
Oracle Linux 8.6 with ONTAP发行版没有已知问题。


NOTE: 有关 Oracle Linux （ Red Hat 兼容内核）的已知问题，请参见 link:hu_rhel_86.html#known-problems-and-limitations["已知问题"] 一节。
