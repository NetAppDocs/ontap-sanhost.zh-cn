...
summary: 介绍如何在 ONTAP 中使用 Oracle Linux 8.2 
---
= 将Oracle Linux 8.2与ONTAP结合使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将以ONTAP为目标的Oracle Linux 8.2配置为目标。
...
对于 Oracle Linux （ OL ） 8.2 ， /etc/multipath.conf 文件必须存在，但您不需要对该文件进行特定更改。OL 8.2 是使用识别和正确管理 ONTAP LUN 所需的所有设置进行编译的。
...
Oracle Linux 8.2 操作系统经过编译，可以识别 ONTAP LUN ，并自动为 ASA 和非 ASA 配置正确设置所有配置参数。
...
NOTE: 要配置Oracle Linux 8.2 Red Hat Enterprise Kernel (RHCK)、请使用link:hu_rhel_82.html#recommended-settings["建议设置"]适用于Red Hat Enterprise Linux (RHEL) 8.2的。



== 已知问题

Oracle Linux 8.2 with ONTAP发行版没有已知问题。


NOTE: 有关 Oracle Linux （ Red Hat 兼容内核）的已知问题，请参见 link:hu_rhel_82.html#known-problems-and-limitations["已知问题"] 适用于 Red Hat Enterprise Linux （ RHEL ） 8.2 。
