...
summary: 在 Oracle Linux 8.2 主机上安装 Linux Host Utilities 以使用 ONTAP LUN 管理 FCP 和 iSCSI 协议操作。 
---
= 使用 ONTAP 存储为 FCP 和 iSCSI 配置 Oracle Linux 8.2
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在 Oracle Linux 8.2 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 来帮助您管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以在Oracle Linux 8.2中使用多路径来管理ONTAP LUN。


NOTE: 您可以使用link:hu_rhel_82.html#rhel-rhck["Red Hat Enterprise Linux (RHEL) 8.2 的推荐设置"]为 Oracle Linux 8.2 配置 Red Hat 兼容内核。
