...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将CentOS 7.5配置为目标、并使用ONTAP。
...
CentOS 7.5操作系统经过编译、可识别ONTAP LUN、并自动为ASA和非ASA配置正确设置所有配置参数。 `multipath.conf`要启动多路径守护进程、必须存在该文件。如果此文件不存在、您可以使用以下命令创建一个空的零字节文件：
