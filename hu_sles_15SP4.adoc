...
下表显示了ONTAP LUN的关键多路径参数以及所需值。如果主机连接到其他供应商的LUN、并且这些参数中的任何一个被覆盖、则需要在中的后续步骤中进行更正 `multipath.conf` 具体适用于ONTAP LUN。如果不执行此操作， ONTAP LUN 可能无法按预期工作。只有在与 NetApp 和 / 或操作系统供应商协商后，才应覆盖这些默认值，并且只有在完全了解影响的情况下才应覆盖这些默认值。
