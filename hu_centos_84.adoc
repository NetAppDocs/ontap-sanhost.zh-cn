...
summary: 介绍如何在ONTAP 中使用CentOS 8.4 
---
= 将CentOS 8.4与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将CentOS 8.4配置为目标并使用ONTAP。
...
对于CentOS 8.4、必须存在`/etc/multipath.conf`文件、但您不需要对该文件进行特定更改。CentOS 8.4是使用识别和正确管理ONTAP LUN所需的所有设置进行编译的。
...
CentOS 8.4经过编译、可以识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
