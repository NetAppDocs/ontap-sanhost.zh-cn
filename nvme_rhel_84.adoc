...
Y
----
* 验证适当的 nve-of 设置， `model` set to `NetApp ONTAP Controller` and `load balancing OPoliciy` set to `round-robin` ，以使相应的 ONTAP 命名空间正确反映在主机上：
+
[listing]
----
...
. 验证 NVMe/FC ONTAP 命名空间是否正确反映在主机上。
+
示例
...
4 KiB + 0 B  FFFFFFFF
----
. 验证每个路径的控制器状态是否为活动状态且是否具有正确的 ANA 状态。
+
示例
