...
---
= 使用ONTAP存储为 NVMe-oF 配置 RHEL 9.4
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 RHEL 9.4 配置 NVMe over Fabrics (NVMe-oF) 主机。有关更多支持和功能信息，请参阅link:hu-nvme-index.html["NVME-oF 概述"^]。

带有 RHEL 9.4 的 NVMe-oF 没有已知的限制。
...
.步骤
. 在服务器上安装 RHEL 9.4。安装完成后，验证您是否正在运行所需的 RHEL 9.4 内核：
+
[source, cli]
----
uname -r
...
从ONTAP 9.12.1 开始，RHEL 9.4 主机和ONTAP控制器之间通过 NVMe/TCP 支持安全带内身份验证。
