...
summary: 在 SUSE Linux Enterprise Server 15 SP1 上安装 Linux Host Utilities 以使用 ONTAP LUN 管理 FCP 和 iSCSI 协议操作。 
---
= 为带有ONTAP存储的FCP和iSCSI配置SUSE Linux Enterprise Server 15 SP1
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在SUSE Linux Enterprise Server 15 SP1主机上安装Linux Host Utilities时、您可以使用Host Utilities帮助您管理使用ONTAP LUN的FCP和iSCSI协议操作。
...
您可以在SUSE Linux Enterprise Server 15 SP1中使用多路径来管理ONTAP LUN。
...
带有ONTAP存储版本的SUSE Linux Enterprise Server 15 SP1存在以下已知问题：
