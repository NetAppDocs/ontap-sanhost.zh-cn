...
summary: 在 CentOS 8.2 主机上安装 Linux Host Utilities 以管理使用ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 为 FCP 和 iSCSI 配置 CentOS 8.2 以用于 ONTAP 存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在 CentOS 8.2 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 来帮助您管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 CentOS 8.2 的多路径来管理 ONTAP LUN。
...
对于带有 ONTAP 存储的 FCP 和 iSCSI，CentOS 8.2 没有已知问题。

有关 CentOS Red Hat 兼容内核的已知问题，请参阅link:hu_rhel_82.html#known-problems-and-limitations["已知问题"]适用于 Red Hat Enterprise Linux (RHEL) 8.2。
