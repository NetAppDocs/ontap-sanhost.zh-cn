...
summary: 在 Oracle Linux 10 主机上安装 Linux Host Utilities，以管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 配置 Oracle Linux 10 以支持 FCP 和 iSCSI 以及 ONTAP 存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 软件为连接到 ONTAP 存储的 Linux 主机提供管理和诊断工具。在 Oracle Linux 10 主机上安装 Linux Host Utilities 后，您可以使用 Host Utilities 来帮助管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 Oracle Linux 10 的多路径功能来管理 ONTAP LUN。
