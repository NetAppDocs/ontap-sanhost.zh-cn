...
summary: 在 Oracle Linux 10.x 主机上安装 Linux Host Utilities，以管理使用 ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 使用 ONTAP 存储为 FCP 和 iSCSI 配置 Oracle Linux 10.x
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 软件为连接到 ONTAP 存储的 Linux 主机提供管理和诊断工具。在 Oracle Linux 10.x 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 帮助您通过 ONTAP LUN 管理 FCP 和 iSCSI 协议操作。
...
link:hu_luhu_71.html["为ONTAP存储安装 Linux Host Utilities 7.1"]。
...
您可以在 Oracle Linux 10.x 中使用多路径来管理 ONTAP LUN。
