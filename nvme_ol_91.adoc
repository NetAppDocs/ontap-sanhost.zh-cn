...
| 1539101 | Oracle Linux 9.1 NVMe-oF主机无法创建永久性发现控制器 | 在Oracle Linux 9.1 NVMe-oF主机上、您可以使用 `nvme discover -p` 用于创建永久性发现控制器(POC)的命令。使用此命令时、应为每个启动程序-目标组合创建一个PDC。但是、如果您在NVMe-oF主机上运行Oracle Linux 9.1、则在中创建PDC将失败 `nvme discover -p` 已执行命令。 | link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18196["18196"^] 
|===