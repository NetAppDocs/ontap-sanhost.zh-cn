...
---
= 将Oracle Linux 9.1与NetApp ONTAP结合使用
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
|===
.示例
以下示例说明了如何更正覆盖的默认值。在这种情况下、将显示 `multipath.conf` 文件定义的值 `path_checker` 和 `no_path_retry` 与ONTAP LUN不兼容的LUN。如果由于其他 SAN 阵列仍连接到主机而无法删除这些参数，则可以专门针对具有设备实例的 ONTAP LUN 更正这些参数。
...
NOTE: 要配置Oracle Linux 9.1 RedHat Enterprise Kernel (RHCK)、请使用 link:hu_rhel_91.html#recommended-settings["建议设置"] 适用于Red Hat Enterprise Linux (RHEL) 9.1。
...
具有NetApp ONTAP版本的Oracle Linux 9.1存在以下已知问题：
