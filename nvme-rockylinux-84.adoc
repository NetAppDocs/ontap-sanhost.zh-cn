...
summary: 如何使用 ONTAP 为 Rocky Linux 8.4 配置 NVMe-oF 主机 
---
= 使用 NVMe-oF 为 ONTAP 存储配置 Rocky Linux 8.4
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.4 主机支持具有非对称命名空间访问 (ANA) 的 NVMe/FC 和 NVMe/TCP 协议。ANA相当于 iSCSI 和 FCP 环境中的非对称逻辑单元访问 (ALUA) 多路径，并使用内核 NVMe 多路径功能实现。

有关支持的配置的更多详细信息，请参见link:https://mysupport.netapp.com/matrix/["互操作性表工具"^]。

.关于此任务
您可以将以下支持和功能与 Rocky Linux 8.4 的 NVMe-oF 主机配置一起使用。在开始配置过程之前、您还应查看已知限制。
...
* 已知限制：
+
** 对于 Rocky Linux 8.4 NVMe-oF 主机，内核 NVMe 多路径默认处于禁用状态。因此，您需要手动启用它。
** 在 Rocky Linux 8.4 主机上，由于存在未解决的问题，NVMe/TCP 是一项技术预览功能。有关详细信息、请参见 https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.4_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.4 发行说明"^] 。
...
使用以下步骤验证最低支持的 Rocky Linux 8.4 软件版本。

.步骤
. 在服务器上安装 Rocky Linux 8.4。安装完成后，验证您是否正在运行指定的 Rocky Linux 8.4 内核：
+
[source, cli]
----
uname -r
...
带有 ONTAP 版本的 Rocky Linux 8.4 上的 NVMe-oF 主机配置没有已知问题。
