...
summary: 介绍如何将 CentOS 6.6 与 ONTAP 结合使用 
---
= 将CentOS 6.6与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将CentOS 6.6配置为目标、并使用ONTAP。
...
对于 CentOS 6.6 ，必须存在 ` /etc/multipath.conf` 文件，但您不需要对该文件进行特定更改。CentOS 6.6 会使用识别和正确管理 ONTAP LUN 所需的所有设置进行编译。要启用 ALUA 处理程序，请执行以下步骤：
...
系统会对 CentOS 6.6 操作系统进行编译，以识别 ONTAP LUN ，并自动为 ASA 和非 ASA 配置正确设置所有配置参数。
...
有关 CentOS （ Red Hat 兼容内核）的已知问题，请参见 link:hu_rhel_66.html#known-problems-and-limitations["已知问题"] 适用于 Red Hat Enterprise Linux （ RHEL ） 6.6 。
