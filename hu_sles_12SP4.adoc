...
---
= 将 SUSE Linux Enterprise Server 12 SP4 与 NetApp ONTAP 结合使用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
对 SUSE Linux Enterprise Server 12 SP4 操作系统进行了编译，以识别 ONTAP LUN 并自动正确设置所有配置参数。要启动多路径守护进程，必须存在 `multipath.conf` 文件，但您可以使用命令 `touch /etc/multipath.conf` 创建一个空的零字节文件
...
对于 SUSE Linux Enterprise Server 12 SP4 ，没有已知问题。
