...
summary: 介绍如何将 Oracle Linux 8.x 与ONTAP结合使用 
---
= 配置 Oracle Linux 8.x 以支持 FCP 和 iSCSI 以及ONTAP存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 软件为连接到ONTAP存储的 Linux 主机提供管理和诊断工具。在 Oracle Linux 8.x 主机上安装 Linux 主机实用程序后，您可以使用主机实用程序来帮助您管理ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 Oracle Linux 8.x 的多路径功能来管理ONTAP LUN。


NOTE: 你可以使用 link:hu-rhel-8x.html#rhel-rhck["Red Hat Enterprise Linux (RHEL) 8.x 的推荐设置"] 为 Oracle Linux 8.x 配置 Red Hat 兼容内核。
