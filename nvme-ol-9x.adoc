...
summary: 适用于 Oracle Linux 9.x 和ONTAP的 NVMe-oF 主机配置 
---
...
了解如何为 Oracle Linux 9.x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-ol-supported-features.html["Oracle Linux ONTAP支持和功能"]。
...
.步骤
. 在服务器上安装Oracle Linux 9.x。安装完成后，请确认您运行的是指定的 Oracle Linux 9.x 内核。
+
[source, cli]
----
uname -r
...
----
. 在 Oracle Linux 9.x 主机上，检查 `hostnqn`字符串 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
NOTE: 从 ONTAP 9.16.1 开始，NVMe/FV 和 NVMe/TCP 报告 ASA r2 系统上的所有优化路径。
