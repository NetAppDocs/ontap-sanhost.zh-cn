...
---
= 使用ONTAP存储为 FCP 和 iSCSI 配置 Ubuntu 24.04
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 软件为连接到ONTAP存储的 Linux 主机提供管理和诊断工具。在 Ubuntu 24.04 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 帮助您管理ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 Ubuntu 24.04 的多路径来管理ONTAP LUN。
