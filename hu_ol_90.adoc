...
summary: 在 Oracle Linux 9.0 主机上安装 Linux Host Utilities 以管理使用 ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 使用 ONTAP 存储为 FCP 和 iSCSI 配置 Oracle Linux 9.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在 Oracle Linux 9.0 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 来帮助您管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以在Oracle Linux 9.0中使用多路径来管理ONTAP LUN。


NOTE: 您可以使用link:hu_rhel_90.html#rhel-rhck["Red Hat Enterprise Linux (RHEL) 9.0 的推荐设置"]为 Oracle Linux 9.0 配置 Red Hat 兼容内核。
...
带有 NetApp ONTAP 存储版本的 Oracle Linux 9.0 存在以下已知问题：
...
在SAN主机上执行Linux Host Utilities命令行界面命令sanlun fcp show adapter -v时、此命令失败、并显示一条错误消息、显示无法找到主机总线适配器(HBA)发现所需的库依赖关系：
