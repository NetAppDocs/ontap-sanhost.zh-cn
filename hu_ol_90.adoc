...
summary: 介绍如何在ONTAP 中使用Oracle Linux 9.0 
---
= 将Oracle Linux 9.0与NetApp ONTAP 结合使用
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
安装 NetApp Host Utilities 软件包时，工具包会自动安装。此套件提供sanlun实用程序、可帮助您管理LUN和HBA。sanlun命令可返回有关映射到主机的LUN的信息、多路径以及创建启动程序组所需的信息。

在以下示例中、sanlun lun show命令返回LUN信息。
...
对于Oracle Linux (OL) 9.0 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。对OL 9.0进行了编译、其中包含识别和正确管理ONTAP LUN所需的所有设置。

您可以使用 `multipath -ll` 命令验证 ONTAP LUN 的设置。

以下各节提供了映射到 ASA 和非 ASA 角色的 LUN 的示例多路径输出。
...
Oracle Linux 9.0操作系统经过编译、可以识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
...
NOTE: 要配置Oracle Linux 9.0 RedHat Enterprise Kernel (RHCK)、请使用 link:hu_rhel_86.html#recommended-settings["建议设置"] 适用于Red Hat Enterprise Linux (RHEL) 9.0。
...
ASM镜像可能需要更改Linux多路径设置、以使ASM能够识别问题并切换到备用故障组。ONTAP 上的大多数 ASM 配置都使用外部冗余，这意味着数据保护由外部阵列提供，并且 ASM 不会镜像数据。某些站点使用正常冗余的 ASM 来提供双向镜像，通常在不同站点之间进行镜像。有关详细信息、请参见ONTAP 上的Oracle数据库。
