...
.关于此任务
安装 Linux Host Utilities 时，它不会更改主机上的任何设置或行为。
...
NOTE: Linux Host Utilities 对 libHBAAPI.so 库具有安装依赖关系。即使主机仅用于 iSCSI 配置，您也需要安装上面列出的软件包之一。或者，您可以使用操作系统级别的工具进行 LUN 和路径管理。
