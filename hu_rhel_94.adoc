...
summary: 介绍如何将Red Hat Enterprise Linux 9.4与ONTAP结合使用 
---
= 将Red Hat Enterprise Linux 9.4与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将Red Hat Enterprise Linux 9.4配置为目标ONTAP。
...
对于Red Hat Enterprise Linux (RHEL) 9.4、 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。RHEL 9.4会编译为包含识别和正确管理ONTAP LUN所需的所有设置。
...
对RHEL 9.4操作系统进行编译、可识别ONTAP LUN、并自动为ASA和非ASA配置正确设置所有配置参数。您可以使用以下建议设置进一步优化主机配置的性能。
...
对于运行ONTAP版本的RHEL 9.4、FC主机配置没有已知问题。
