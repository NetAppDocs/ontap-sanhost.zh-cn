...
---
= 适用于采用ONTAP的Ubuntu 22.04的NVMe-oF主机配置
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
--
.步骤
. Ubuntu 22.04 GA内核中包含的本机内置qla2xxx驱动程序具有ONTAP支持所必需的最新上游修复程序。验证您是否正在运行受支持的适配器驱动程序和固件版本：
+
[listing]
----
...
对于采用ONTAP版本的Ubuntu 22.04的NVMe-oF主机配置、没有已知问题。
