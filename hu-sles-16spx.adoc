...
summary: 在 SUSE Linux Enterprise Server 16 上安装 Linux Host Utilities，以管理使用 ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 使用 ONTAP 存储为 FCP 和 iSCSI 配置 SUSE Linux Enterprise Server 16
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主机实用程序软件为连接到 ONTAP 存储的 Linux 主机提供管理和诊断工具。在 SUSE Linux Enterprise Server 16 主机上安装 Linux 主机实用程序时，您可以使用主机实用程序帮助您使用 ONTAP LUN 管理 FCP 和 iSCSI 协议操作。
...
您可以与 SUSE Linux Enterprise Server 16 一起使用多路径来管理 ONTAP LUN。
