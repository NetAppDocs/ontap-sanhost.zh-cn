...
* 在Oracle Linux 8.3中、NVMe和SCSI流量均可在同一主机上运行。这是通常部署的主机配置。您可以照常为SCSI LUN配置dm-Multipath以生成mpath设备、也可以使用NVMe多路径在主机上配置NVMe-oF多路径设备(例如 `/dev/nvmeXnY`)。
...
有关最新的受支持配置列表、请参见 http://mysupport.netapp.com/matrix3["互操作性表工具(IMT)"^]。
...
.步骤
. 在服务器上安装Oracle Linux 8.3 GA。安装完成后、验证您是否正在运行指定的Oracle Linux 8.3 GA内核。有关最新的受支持版本列表、请参见 https://mysupport.netapp.com/matrix/["互操作性表工具"^]。
+
[listing]
----
# uname -r
...
[NOTE]
====
如果要在同一个Oracle Linux 8.3主机上同时运行NVMe和SCSI流量、NetApp建议分别对ONTAP命名空间和ONTAP LUN使用内核NVMe多路径 `dm-multipath`。这也意味着应将ONTAP命名空间列入黑名单、以防止 `dm-multipath`声明这些命名空间 `dm-multipath`设备。为此、可以将_ENGE_Foreign _设置添加到文件中 `/etc/multipath.conf`：
...
.步骤
. 验证您使用的是受支持的适配器。有关支持的适配器的当前列表，请参见 https://mysupport.netapp.com/matrix/["互操作性表工具"^]。
+
[listing]
----
...
Emulex LPe36002-M64 2-Port 64Gb Fibre Channel Adapter
----
. 验证您是否正在使用建议的 Broadcom lpfc 固件和收件箱驱动程序。有关支持的适配器驱动程序和固件版本的最新列表，请参见 https://mysupport.netapp.com/matrix/["互操作性表工具"^]。
+
[listing]
----
