...
summary: 如何使用ONTAP存储为 Rocky Linux 10.0 配置 NVMe-oF 主机 
---
= 使用ONTAP存储为 NVMe-oF 配置 Rocky Linux 10.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 Rocky Linux 10.0 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支持和功能"]。

*带有 Rocky Linux 10.0 的 NVMe-oF 具有以下已知限制：*

* 不支持对 NVMe-oF 的 DM 多路径支持。
...
.步骤
. 在服务器上安装 Rocky Linux 10.0。安装完成后，验证您是否正在运行所需的 Rocky Linux 10.0 内核：
+
[source, cli]
----
uname -r
...
从ONTAP 9.12.1 开始，Rocky Linux 10.0 主机和ONTAP控制器之间通过 NVMe/TCP 支持安全带内身份验证。
...
cat /etc/nvme/hostnqn
----
. 为 Rocky Linux 10.0 主机生成 dhchap 密钥。
+
