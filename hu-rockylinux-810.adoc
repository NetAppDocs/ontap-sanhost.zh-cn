...
---
= 使用 ONTAP 存储为 FCP 和 iSCSI 配置 Rocky Linux 8.10
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在 Rocky Linux 8.10 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 来帮助您管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 Rocky Linux 8.10 的多路径来管理 ONTAP LUN。
