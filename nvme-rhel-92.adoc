...
---
= 使用ONTAP存储为 NVMe-oF 配置 RHEL 9.2
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 RHEL 9.2 配置 NVMe over Fabrics (NVMe-oF) 主机。有关更多支持和功能信息，请参阅link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 9.2 的 NVMe-oF 具有以下已知限制：*
...
.步骤
. 在服务器上安装 RHEL 9.2。安装完成后，验证您是否正在运行所需的 RHEL 9.2 内核：
+
[source, cli]
----
uname -r
