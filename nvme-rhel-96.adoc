...
---
= 使用 ONTAP 存储为 NVMe-oF 配置 RHEL 9.6
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 Red Hat Enterprise Linux (RHEL) 9.6 配置 NVMe over Fabrics (NVMe-oF) 主机。有关更多支持和功能信息，请参阅link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 9.6 的 NVMe-oF 具有以下已知限制：*
...
.步骤
. 在服务器上安装 RHEL 9.6。安装完成后，验证您是否正在运行所需的 RHEL 9.6 内核：
+
[source, cli]
----
uname -r
...
RHEL 9.6 将 NVMe-oF 的默认 iopolicy 设置为 `round-robin`。如果你想将 iopolicy 更改为 `queue-depth`，修改udev规则文件如下：
...
----
. 验证子系统的当前 iopolicy。将 <subsystem> 替换为适当的名称，例如， `nvme-subsys0` 。
+
[source, cli]
----
...
从ONTAP 9.12.1 开始，RHEL 10.0 主机和ONTAP控制器之间通过 NVMe/TCP 支持安全带内身份验证。
