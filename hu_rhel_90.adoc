...
summary: 介绍如何在ONTAP 中使用Red Hat Enterprise Linux 8.6 
---
= 将Red Hat Enterprise Linux 9.0与ONTAP结合使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将ONTAP配置为目标Red Hat Enterprise Linux 9.0。
...
对于Red Hat Enterprise Linux (RHEL) 9.0、必须存在`/etc/multipath.conf`文件、但您不需要对该文件进行特定更改。RHEL 9.0使用识别和正确管理ONTAP LUN所需的所有设置进行编译。
...
RHEL 9.0操作系统经过编译、可识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
