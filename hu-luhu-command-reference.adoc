...
summary: 介绍如何将 Linux 主机实用程序与ONTAP存储配合使用 
---
= 使用 Linux Host Utilities 命令验证ONTAP存储配置
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 Linux Host Utilities 示例命令参考，通过 Host Utilities 工具对NetApp存储配置进行端到端验证。
...
您可以从特定Storage VM (SVM)检索映射到主机的所有LUN的列表。
...
您可以检索特定ONTAP映射到历史记录的LUN SVM标识的列表。
...
您可以检索指定SVM目标LIF WWPN在主机上发现的ONTAP LUN的列表。
