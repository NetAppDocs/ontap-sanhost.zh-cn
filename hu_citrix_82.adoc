...
如果您的主机连接到来自其他供应商的 LUN，并且任何多路径参数设置都被覆盖，则需要通过修改现有节点或在 `/etc/multipath/conf.d/custom.conf`文件中添加专门应用于 ONTAP LUN 的新节点来更正这些设置。您需要在文件中向下滚动，查看适用于您的主机的更改。如果不更正参数，ONTAP LUN 可能无法按预期工作。
