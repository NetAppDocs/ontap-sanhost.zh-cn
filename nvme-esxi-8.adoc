...
* 从 ONTAP 9.16.1 开始，默认情况下，所有新创建的 NVMe 命名空间都已启用空间分配。
* 从 ONTAP 9.9.1 P3 开始，ESXi 8 支持 NVMe/FC 协议。
* 从 ONTAP 9.10.1 开始，ONTAP 支持 NVMe/TCP 协议。
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654,"^] | 当 NVMe/FC 协议与 ONTAP 版本 9.9.1 一起使用时，ONTAP 节点不可操作 | ONTAP 9.9.1 引入了对 NVMe "abort" 命令的支持。当 ONTAP 收到 "abort" 命令以中止正在等待其伙伴命令的 NVMe 融合命令时，会发生 ONTAP 节点中断。此问题仅在使用 NVMe 融合命令（例如 ESX）和光纤通道 (FC) 传输的主机上出现。 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | 如果使用vNVMe适配器的Linux VM遇到较长的All Paths Down (APD)窗口、则会发生I/O错误  a| 
运行 vSphere 8.x 及更高版本并使用虚拟 NVMe (vNVME) 适配器的 Linux VM 会遇到 I/O 错误，因为默认情况下会禁用 vNVMe 重试操作。为了避免在所有路径关闭 (APD) 或繁重 I/O 负载期间运行较旧内核的 Linux VM 上发生中断，VMware 引入了可调节的 "VSCSIDisableNvmeRetry" 来禁用 vNVMe 重试操作。
