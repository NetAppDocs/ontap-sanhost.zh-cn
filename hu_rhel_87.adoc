...
---
= 将Red Hat Enterprise Linux 8.7与NetApp ONTAP 结合使用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
RHEL 8.7操作系统经过编译、可识别ONTAP LUN、并自动为ASA 和非ASA配置正确设置所有配置参数。
...
| 1501048 | FC传输层超时参数 `dev_loss_tmo` 对于远程SCSI目标端口、报告负值(-1) |  `dev_loss_tmo` timeout参数表示FC传输为防止远程端口丢失而应隔离的默认秒数。如果超过此值、则会从主机中删除SCSI目标。此参数值应为正整数值。但是、在RHEL 8.7中、 `sysfs` 文件系统将为返回负值(-1) `dev_loss_tmo` SCSI设备的超时参数。下面是用于验证的命令行示例 `dev_loss_tmo` 运行正常的主机上的参数：
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`。在RHEL 8.7中、此问题描述 没有临时解决策。 | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
