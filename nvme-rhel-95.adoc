...
了解如何为 Red Hat Enterprise Linux (RHEL) 9.5 配置 NVMe over Fabrics (NVMe-oF) 主机。有关更多支持和功能信息，请参阅link:hu-nvme-index.html["NVME-oF 概述"^]。

RHEL 9.5 的 NVMe-oF 没有已知的限制。
...
.步骤
. 在服务器上安装 RHEL 9.5。安装完成后，验证您是否正在运行所需的 RHEL 9.5 内核：
+
[source, cli]
----
uname -r
