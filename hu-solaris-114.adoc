...
** NetApp 支持使用 Solaris 虚拟化技术，其中 Oracle 支持整体配置，并且在受支持的配置中，可直接访问 LUN 的任何分区均列于 link:https://imt.netapp.com/matrix/#welcome["IMT"^] 中。这包括根容器、LDOM I/O 域以及使用 NPIV 访问 LUN 的 LDOM。
...
+
.. 确认您的 Solaris 主机已安装最新的支持存储库更新 (SRU)：
+
[source, cli]
----
pkg info entire
