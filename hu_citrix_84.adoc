...
summary: 介绍如何将 Citrix XenServer 8.4 与 ONTAP 结合使用 
---
...
[role="lead"]
"Linux Host Utilities (Linux主机实用程序)"软件为连接到ONTAP存储的Linux主机提供管理和诊断工具。在 Citrix Xenserver 8.4 主机上安装 Linux Host Utilities 时，您可以使用 Host Utilities 帮助您管理 ONTAP LUN 的 FCP 和 iSCSI 协议操作。
...
您可以使用 Citrix Xenserver 8.4 的多路径来管理 ONTAP LUN。
