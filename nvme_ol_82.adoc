...
从ONTAP 9.6开始、Oracle Linux 8.2支持NVMe/FC。Oracle Linux 8.2主机可以通过同一个光纤通道(Fibre Channel、FC)启动程序适配器端口同时运行NVMe/FC和FCP流量。有关支持的FC适配器和控制器的列表、请参见 link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 。有关支持的配置的当前列表，请参见link:https://mysupport.netapp.com/matrix/["互操作性表工具"^]。
