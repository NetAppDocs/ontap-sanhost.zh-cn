...
summary: 介绍如何将Ubuntu 20.04与ONTAP结合使用 
---
= 将Ubuntu 20.04与ONTAP结合使用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
您可以使用ONTAP SAN主机配置设置将ONTAP配置为目标Ubuntu 20.04。


NOTE: NetApp Linux Unified Host Utilities软件包不适用于Ubuntu 20.04操作系统。
...
对于Ubuntu 20.04、 `/etc/multipath.conf` 文件必须存在、但您不需要对文件进行特定更改。Ubuntu 20.04包含识别和正确管理ONTAP LUN所需的所有设置。
...
对Ubuntu 20.04操作系统进行编译、可识别ONTAP LUN、并自动为ASA和非ASA配置正确设置所有配置参数。您可以使用以下建议设置进一步优化主机配置的性能。
