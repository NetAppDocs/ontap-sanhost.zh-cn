...
---
= 使用ONTAP存储为 NVMe-oF 配置 Rocky Linux 8.7
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 Rocky Linux 8.7 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支持和功能"]。

*带有 Rocky Linux 8.7 的 NVMe-oF 具有以下已知限制：*

* 目前不支持使用 NVMe-oF 协议的 SAN 启动。
* Rocky Linux 8.7 中的 NVMe-oF 主机默认禁用内核 NVMe 多路径；您必须手动启用它。
...
.步骤
. 在服务器上安装 Rocky Linux 8.7。安装完成后，验证您是否正在运行所需的 Rocky Linux 8.7 内核：
+
[source, cli]
----
uname -r
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 8.7 NVMe-oF 主机创建重复的持久发现控制器 | 在 NVMe-oF 主机上，您可以使用“nvme discover -p”命令创建持久发现控制器 (PDC)。使用此命令时，每个启动器-目标组合只能创建一个 PDC。但是，如果您在 NVMe-oF 主机上运行 Rocky Linux 8.7，则每次执行“nvme discover -p”时都会创建重复的 PDC。这会导致主机和目标上资源的不必要使用。 
|===