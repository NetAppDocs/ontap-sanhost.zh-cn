...
summary: 在 Windows Server 2019 主机上安装 Windows Host Utilities，以帮助您管理ONTAP LUN 的 FCP 和 iSCSI 协议操作。 
---
= 配置 Windows Server 2019 以支持 FCP 和 iSCSI 以及ONTAP存储
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Windows 主机实用程序使您可以将 Windows 主机连接到NetApp SAN 上的虚拟磁盘 (LUN)。在 Windows Server 2019 主机上安装 Windows 主机实用程序，以帮助您管理ONTAP LUN 的 FCP 和 iSCSI 协议操作。
