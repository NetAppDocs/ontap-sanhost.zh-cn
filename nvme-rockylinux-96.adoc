...
.关于此任务
您可以将以下支持和功能与 Rocky Linux 9.6 的 NVMe-oF 主机配置一起使用。在开始配置过程之前、您还应查看已知限制。
...
** Rocky Linux 9.6 默认为 NVMe 命名空间启用内核 NVMe 多路径，无需明确设置。
** 从 ONTAP 9.12.1 开始，引入了对 NVMe/TCP 的安全带内身份验证的支持。您可以使用 Rocky Linux 9.6 对 NVMe/TCP 进行安全带内身份验证。
+
...
使用以下步骤验证最低支持的 Rocky Linux 9.6 软件版本。

.步骤
. 在服务器上安装 Rocky Linux 9.6。安装完成后，验证您运行的是指定的 Rocky Linux 9.6 内核：
+
[source, cli]
----
uname -r
...
使用 Rocky Linux 9.6，  `nvmefc-boot-connections.service`和 `nvmf-autoconnect.service` NVMe/FC 中包含的启动服务 `nvme-cli`系统启动时，这些软件包会自动启用。启动完成后，请验证这些服务是否已启用。
