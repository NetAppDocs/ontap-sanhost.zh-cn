...
summary: 如何为 Rocky Linux 10.x 配置带有ONTAP存储的 NVMe-oF 主机 
---
...
了解如何为 Rocky Linux 10.x 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支持和功能"]。
...
.步骤
. 在服务器上安装 Rocky Linux 10.x。安装完成后，请确认您运行的是所需的 Rocky Linux 10.x 内核：
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. 为 Rocky Linux 10.x 主机生成 dhchap 密钥。
+
