...
---
= 使用ONTAP存储为 NVMe-oF 配置 Rocky Linux 9.1
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何为 Rocky Linux 9.1 配置 NVMe over Fabrics (NVMe-oF) 主机。如需更多支持和功能信息，请参阅 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支持和功能"]。

*带有 Rocky Linux 9.1 的 NVMe-oF 具有以下已知限制：*
...
.步骤
. 在服务器上安装 Rocky Linux 9.1。安装完成后，验证您是否正在运行所需的 Rocky Linux 9.1 内核：
+
[source, cli]
----
uname -r
