...
summary: 如何使用 ONTAP 为 Rocky Linux 9.1 配置 NVMe-oF 主机 
---
= 使用 NVMe-oF 为 ONTAP 存储配置 Rocky Linux 9.1
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 9.1 主机支持具有非对称命名空间访问 (ANA) 的 NVMe/FC 和 NVMe/TCP 协议。ANA相当于 iSCSI 和 FCP 环境中的非对称逻辑单元访问 (ALUA) 多路径，并使用内核 NVMe 多路径功能实现。

有关支持的配置的更多详细信息，请参见link:https://mysupport.netapp.com/matrix/["互操作性表工具"^]。

.关于此任务
您可以将以下支持和功能与 Rocky Linux 9.1 的 NVMe-oF 主机配置一起使用。在开始配置过程之前、您还应查看已知限制。
...
* 可用功能：
+
** Rocky Linux 9.1 默认为 NVMe 命名空间启用内核 NVMe 多路径，无需明确设置。
...
使用以下步骤验证最低支持的 Rocky Linux 9.1 软件版本。

.步骤
. 在服务器上安装 Rocky Linux 9.1。安装完成后，验证您运行的是指定的 Rocky Linux 9.1 内核：
+
[source, cli]
----
uname -r
...
带有 ONTAP 存储的 Rocky Linux 9.1 的 NVMe-oF 主机配置存在以下已知问题：
...
| 1503468 | `nvme list-subsys`命令返回给定子系统列出的重复的 NVMe 控制器 | 这 `nvme list-subsys`命令应返回与给定子系统关联的 NVMe 控制器的唯一列表。在 Rocky Linux 9.1 中，  `nvme list-subsys`该命令会返回属于指定子系统的所有命名空间的 NVMe 控制器及其各自的 ANA 状态。但是，ANA 状态是每个命名空间的属性，因此，如果您列出给定命名空间的子系统命令语法，则应该显示具有路径状态的唯一 NVMe 控制器条目。 
|===